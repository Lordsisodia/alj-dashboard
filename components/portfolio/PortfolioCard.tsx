...
export const PortfolioCard = ({ project, onImageClick, onProjectClick, loading }: PortfolioCardProps) => {
  ...
  const images = (project?.images && project.images.length > 0) ? project.images : ['/placeholder-project.jpg'];
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -5 }}
      className={cn("group h-full", onProjectClick ? "cursor-pointer" : "")}
      onClick={() => {
    ...
    >
      <Card className="h-full flex flex-col overflow-hidden border border-siso-border bg-siso-bg-secondary hover:border-siso-orange/40 shadow-[0_18px_38px_-24px_rgba(0,0,0,0.65)] hover:shadow-[0_22px_54px_-28px_rgba(255,120,60,0.28)] transition-all duration-300">
        {/* Image Carousel */}
        <div className="relative h-48 overflow-hidden bg-siso-bg">
          <AnimatePresence mode="wait">
            <motion.img
              ...
              alt={`${project?.app_name ?? 'Project'} - Image ${currentImageIndex + 1}`}
          ...
          {images.length > 1 && (
            <div className="absolute inset-0 flex items-center justify-between p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <Button
                size="icon"
                variant="ghost"
                onClick={(e) => { e.stopPropagation(); prevImage(e as React.MouseEvent); }}
              ...
              <Button
                size="icon"
                variant="ghost"
                onClick={(e) => { e.stopPropagation(); nextImage(e as React.MouseEvent); }}
                className="h-8 w-8 bg-black/50 text-white hover:bg-black/70"
              >
          ...
          <div className="absolute top-3 right-3">
            <Badge className={cn("text-xs", getStatusColor(project?.development_status ?? 'planning'))}>
              {(project?.development_status ?? 'planning').replace('_', ' ').toUpperCase()}
            </Badge>
          </div>
        </div>
          ...
          {project.testimonial && (
            <div className="p-3 bg-siso-bg/50 rounded-lg border border-siso-border/50">
              <p className="text-xs text-siso-text-muted italic line-clamp-2 mb-2">
                "{project.testimonial.text ?? project.testimonial.quote ?? ''}"
              </p>
              <div className="text-xs text-siso-orange font-medium">
                — {project.testimonial.author}
                {project.testimonial.role || project.testimonial.title ? (
                  <span>, {project.testimonial.role ?? project.testimonial.title}</span>
                ) : null}
              </div>
            </div>
          )}
        ...
        <CardFooter className="pt-4 space-y-3">
          <div className="flex gap-2 w-full">
            ...
            {project.case_study_url && (
              <Button
                ...
                onClick={(e) => { e.stopPropagation(); window.open(project.case_study_url!, '_blank'); }}
              >
          ...
          {/* Project Value */}
          {project.estimated_value !== undefined && (
