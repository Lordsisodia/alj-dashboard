      ...
      >
        ...
        <div
          className={`flex-1 relative transition-opacity duration-500 ${
            isCollapsed ? "opacity-0 w-0" : "opacity-100 ml-3 pr-3"
          }`}
          style={{ transitionTimingFunction: softSpringEasing }}
        >
        ...
        {!isCollapsed && (
          <div className="flex items-center gap-1 ml-2 shrink-0">
            <button
              type="button"
              className="p-1 rounded-full text-neutral-500 hover:text-white hover:bg-neutral-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-siso-orange/60 transition opacity-80 group-hover:opacity-100"
              onClick={(e) => {
                e.stopPropagation();
                onInfoClick?.(item);
              }}
  ...
  if (section.isCallout && section.items.length > 0) {
    ...
    return (
      <button type="button" className="w-full text-left" onClick={() => onItemClick?.(primary)}>
        <div className="rounded-[24px] border border-white/15 bg-gradient-to-br from-[#1a1106] via-[#0f0f0f] to-[#050505] px-4 py-4 transition hover:border-siso-orange/50">
  ...
  return (
    <div
      className={cn(
        ...
        section.isCallout && "rounded-[20px] border border-white/10 bg-black/25 px-2 py-3 text-white",
      )}
    >
      {!section.hideTitle && (
        <div
