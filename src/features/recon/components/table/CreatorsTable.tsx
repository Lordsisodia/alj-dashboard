  ...
  function renderHeader() {
    return (
      <div className="grid" style={{ gridTemplateColumns: gridCols, height: 36, backgroundColor: '#f9f9f9', borderBottom: '1px solid rgba(0,0,0,0.10)' }}>
        ...
        <ColumnFilterHeader label="Creator" icon={<Users size={9} />} style={hdrCell} tooltip="Handle and display name with niche tag." />
        {visibleCols.map(col => {
          if (col.key === 'health')    return <ColumnFilterHeader key={col.key} label="Profile" icon={<Star size={9} />} options={HEALTH_OPTS} value={filters.health} onChange={v => set('health', v as string)} style={hdrCell} tooltip="Overall profile quality score based on bio, links, story highlights, and post consistency." />;
          if (col.key === 'niche')     return <ColumnFilterHeader key={col.key} label="Niche" icon={<Hash size={9} />} options={NICHE_OPTS} multi value={filters.niche} onChange={v => set('niche', v as string[])} style={hdrCell} tooltip="Content category this creator posts in." />;
          if (col.key === 'followers') return <RangeFilterHeader key={col.key} label="Followers" icon={<TrendingUp size={9} />} min={filters.followers.min} max={filters.followers.max} step={1000} align="right" onChange={v => set('followers', v)} style={hdrCell} tooltip="Total follower count. Filter by range to target a specific tier." />;
          if (col.key === 'avgViews')  return <ColumnFilterHeader key={col.key} label="Latest Views" icon={<Eye size={9} />} options={[]} value="" onChange={() => {}} style={hdrCell} tooltip="Average views across recently scraped posts." />;
          if (col.key === 'following') return <RangeFilterHeader key={col.key} label="Following" icon={<Users size={9} />} min={filters.following.min} max={filters.following.max} step={100} align="right" onChange={v => set('following', v)} style={hdrCell} tooltip="Number of accounts this creator follows. A high following-to-follower ratio may indicate non-organic growth." />;
          if (col.key === 'posts')     return <ColumnFilterHeader key={col.key} label="Posts" icon={<BarChart2 size={9} />} options={POSTS_OPTS} value={filters.posts} align="right" onChange={v => set('posts', v as string)} style={hdrCell} tooltip="Total posts published on the profile." />;
          if (col.key === 'engRate')  return <RangeFilterHeader key={col.key} label="Eng. Rate" icon={<Zap size={9} />} min={filters.engRate.min} max={filters.engRate.max} step={0.1} suffix="%" align="right" onChange={v => set('engRate', v)} style={hdrCell} tooltip="Engagement rate (likes + comments ÷ followers × 100). Filter to find high-engagement creators." />;
          if (col.key === 'score')    return <RangeFilterHeader key={col.key} label="Score" icon={<Award size={9} />} min={filters.score.min} max={filters.score.max} step={1} align="right" onChange={v => set('score', v)} style={hdrCell} tooltip="ISSO composite creator score based on reach, engagement, content signals, and outlier ratio." />;
          if (col.key === 'outlierRatio') return <ColumnFilterHeader key={col.key} label="Outlier" icon={<TrendingUp size={9} />} options={[]} value="" onChange={() => {}} style={hdrCell} tooltip="How often this creator's posts beat their own baseline - higher means more consistent viral potential." />;
          if (col.key === 'postsThisWeek') return <ColumnFilterHeader key={col.key} label="Posts/Wk" icon={<BarChart2 size={9} />} options={[{ value: '0', label: '0' }, { value: '1-3', label: '1-3' }, { value: '4+', label: '4+' }]} value="" align="right" onChange={() => {}} style={hdrCell} tooltip="Average posts published per week based on recent activity." />;
          if (col.key === 'category')  return <ColumnFilterHeader key={col.key} label="IG Category" icon={<Globe size={9} />} options={[{ value: 'Personal blog', label: 'Personal blog' }, { value: 'Business', label: 'Business' }, { value: 'Entertainment', label: 'Entertainment' }, { value: 'Sports', label: 'Sports' }, { value: 'Other', label: 'Other' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Instagram business category set on the profile." />;
          if (col.key === 'bio')        return <ColumnFilterHeader key={col.key} label="Bio" icon={<AlignLeft size={9} />} options={[]} value="" onChange={() => {}} style={hdrCell} tooltip="Profile bio text scraped from Instagram." />;
          if (col.key === 'linkInBio') return <ColumnFilterHeader key={col.key} label="Link" icon={<ExternalLink size={9} />} options={[{ value: 'yes', label: 'Has Link' }, { value: 'no', label: 'No Link' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Whether the creator has a URL in their bio." />;
          if (col.key === 'verified')  return <ColumnFilterHeader key={col.key} label="Verified" icon={<CheckCircle size={9} />} options={[{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Instagram blue checkmark verification status." />;
          if (col.key === 'private')   return <ColumnFilterHeader key={col.key} label="Private" icon={<Lock size={9} />} options={[{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Whether the account is set to private." />;
          if (col.key === 'enrichStatus') return <ColumnFilterHeader key={col.key} label="Enrich" icon={<Zap size={9} />} options={[{ value: 'done', label: 'Done' }, { value: 'idle', label: 'Idle' }, { value: 'error', label: 'Error' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Data enrichment status - whether the full profile has been scraped." />;
          if (col.key === 'source')    return <ColumnFilterHeader key={col.key} label="Source" icon={<Globe size={9} />} options={[{ value: 'scraper', label: 'Scraper' }, { value: 'pre_approved', label: 'Pre-approved' }, { value: 'manual', label: 'Manual' }]} value="" onChange={() => {}} style={hdrCell} tooltip="How this creator was added (Scraper, Manual, or Pre-approved)." />;
          if (col.key === 'igtvVideoCount') return <ColumnFilterHeader key={col.key} label="IGTV" icon={<Tv size={9} />} options={[{ value: 'yes', label: 'Has IGTV' }, { value: 'no', label: 'None' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Number of IGTV / long-form videos published on the profile." />;
          if (col.key === 'highlightReels') return <ColumnFilterHeader key={col.key} label="Highlights" icon={<Star size={9} />} options={[{ value: 'yes', label: 'Has Highlights' }, { value: 'no', label: 'None' }]} value="" onChange={() => {}} style={hdrCell} tooltip="Number of Instagram Story highlight reels on the profile." />;
          return null;
        })}
