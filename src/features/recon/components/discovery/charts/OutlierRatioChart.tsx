...
function buildBuckets(candidates: MappedCandidate[]): Bucket[] {
  ...
  for (const c of candidates) {
    const r = c.outlierRatio;
    if      (r < 1) buckets[0][2]++;
    else if (r < 2) buckets[1][2]++;
    else if (r < 3) buckets[2][2]++;
    else if (r < 5) buckets[3][2]++;
    else             buckets[4][2]++;
  }
...
const chartConfig = {
  outliers: { label: 'candidates', color: '#dc2626' },
} satisfies ChartConfig;

export function OutlierRatioChart({ candidates }: Props) {
    ...
    <Card className="border-[rgba(0,0,0,0.07)]">
      <CardHeader className="p-4 pb-0">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xs font-semibold text-neutral-500 uppercase tracking-wide leading-none">Outlier Ratio</CardTitle>
      ...
      <CardContent className="p-4 pt-2">
        ...
        <div className="flex justify-between mt-1">
          {data.map(b => (
            <span key={b.label} className="text-[8px] text-neutral-400">{b.label}</span>
          ))}
        </div>
      </CardContent>
    </Card>
