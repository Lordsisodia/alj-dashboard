...
function ScrapeButton({ handle, onScrapeComplete, onScrapeStart, onScrapeEnd }: {
  ...
  async function handleScrape(e: React.MouseEvent) {
    ...
    try {
      ...
      // Fire immediately so item leaves Approved and Scraping clears right away
      setScraping(false);
      onScrapeEnd?.();
...
export function ApprovedRow({ candidate, onSelect, onScrapeComplete, onScrapeStart, onScrapeEnd }: Props) {
  return (
    <div
      onClick={onSelect}
      className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg cursor-pointer transition-all hover:brightness-100"
