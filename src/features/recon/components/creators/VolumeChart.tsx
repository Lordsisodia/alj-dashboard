    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-neutral-400 mt-0.5">Last 14 days — all creators</p>
        </div>
        <div
          ...
          {data.map((point, i) => {
            ...
            return (
              ...
              >
                ...
                <motion.div
                  initial={{ height: 0 }}
                  animate={isInView ? { height: `${Math.max(pct, 3)}%` } : { height: 0 }}
        ...
        <div className="absolute left-12 right-0 bottom-0 h-6 flex items-end gap-1">
          {data.map((point, i) => (
            <div key={point.label} className="flex-1 text-center text-[9px] text-neutral-400 truncate px-0.5">
              {i % 2 === 0 ? point.label.replace('Mar ', 'M').replace('Apr ', 'A') : ''}
            </div>
          ))}
        </div>
      </div>
