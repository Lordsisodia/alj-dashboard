...
function StatPill({ label, value }: { label: string; value: string | number }) {
  ...
  return (
    <div className="ml-3 flex-shrink-0 flex items-center gap-1.5 bg-white border border-gray-500/20 rounded-2xl px-2.5 py-0.5">
      <span className="text-[11px] text-neutral-500 font-medium">{label}</span>
      <span className="w-px h-3 bg-neutral-300 flex-shrink-0" />
      <span className="text-[11px] font-semibold text-neutral-700">{formatted}</span>
    </div>
  );
}
      ...
      >
        ...
        <div ref={dropdownRef} className="relative flex-shrink-0">
          ...
          >
            <button
              onClick={onAction}
              className="flex items-center gap-1.5 px-4 h-full text-sm font-semibold text-white hover:brightness-105 transition-all active:scale-95 whitespace-nowrap"
            >
