...
import type { ColDef, ColVisibility } from './types';

function ToggleRow<ColKey extends string>({
  col,
  checked,
  onChange,
}: {
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2">
      <div className="flex items-center gap-2 min-w-0">
        <span className="text-xs text-neutral-700 truncate">{col.label}</span>
        {col.badge && (
          <span className="text-[9px] font-medium px-1 py-0.5 rounded bg-violet-50 text-violet-500 flex-shrink-0">
            {col.badge}
          </span>
        )}
      </div>
      <button
        ...
        className={cn(
          'relative w-8 h-4 rounded-full transition-colors duration-200 flex-shrink-0 ml-2',
...
interface ColumnVisibilityPillProps<ColKey extends string> {
  columns: ColDef<ColKey>[];
  value: ColVisibility<ColKey>;
  onChange: (v: ColVisibility<ColKey>) => void;
}

export function ColumnVisibilityPill<ColKey extends string>({
  columns,
  value,
  onChange,
}: ColumnVisibilityPillProps<ColKey>) {
  ...
  const visibleCount = hideableCols.filter(c => value[c.key]).length;
